...
export function PaymentsWidget({ data }: { data: DashboardData }) {
  ...
  const METHOD_CFG: Record<string, { labelKey: string; className: string }> = {
    BANK_TRANSFER: { labelKey: "methodBankTransfer", className: "bg-blue-50 text-blue-700 border-blue-200" },
    CHECK:         { labelKey: "methodCheck",        className: "bg-violet-50 text-violet-700 border-violet-200" },
    CASH:          { labelKey: "methodCash",         className: "bg-emerald-50 text-emerald-700 border-emerald-200" },
    CREDIT_CARD:   { labelKey: "methodCreditCard",   className: "bg-orange-50 text-orange-700 border-orange-200" },
    OTHER:         { labelKey: "methodOther",        className: "bg-gray-50 text-gray-600 border-gray-200" },
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="border-t border-border/40 px-4 py-2 shrink-0">
        <Button variant="ghost" size="sm" asChild className="text-xs text-muted-foreground w-full justify-end">
          <Link href="/finance">
            {t("toFinance")} <ChevronLeft className="h-3.5 w-3.5 ms-1" />
          </Link>
        </Button>
      </div>
    </div>
