...
import {
  Plus, Phone, ShieldCheck, ShieldAlert, Clock,
...
// className-only cert config — labels come from translations
const CERT_CLASS: Record<CertStatus, { cls: string; Icon: React.ElementType }> = {
  valid:   { cls: "bg-emerald-100 text-emerald-700 border-emerald-200", Icon: ShieldCheck },
  expired: { cls: "bg-red-100 text-red-700 border-red-200",             Icon: ShieldAlert },
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        ...
        setError(res.error ?? t("edit.updateError"));
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md" dir={locale === "he" ? "rtl" : "ltr"}>
        <DialogHeader>
          <DialogTitle>{t("edit.title", { name: employee.name })}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3 pt-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Input value={form.trade} onChange={(e) => setForm({ ...form, trade: e.target.value })} placeholder={t("form.tradePlaceholder")} />
            </div>
            <div className="space-y-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Input value={form.idNumber} onChange={(e) => setForm({ ...form, idNumber: e.target.value })} placeholder={t("form.idPlaceholder")} />
            </div>
            <div className="space-y-1">
          ...
          <div className="flex justify-end gap-2 pt-1">
            <Button variant="outline" onClick={() => { onOpenChange(false); setError(null); }}>{tCommon("cancel")}</Button>
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <Label htmlFor="show-inactive" className="cursor-pointer">{t("showInactive")}</Label>
        </div>
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-md" dir={locale === "he" ? "rtl" : "ltr"}>
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="flex justify-end gap-2 pt-1">
                  <Button variant="outline" onClick={() => { setOpen(false); setForm(EMPTY); setError(null); }}>{tCommon("cancel")}</Button>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          { labelKey: "stats.active",      value: employees.filter((e) => e.active).length,                           color: "text-emerald-600" },
          { labelKey: "stats.inactive",    value: employees.filter((e) => !e.active).length,                          color: "text-slate-400"   },
          { labelKey: "stats.certValid",   value: employees.filter((e) => getCertStatus(e) === "valid").length,        color: "text-emerald-600" },
          { labelKey: "stats.certExpired", value: employees.filter((e) => getCertStatus(e) === "expired").length,      color: "text-red-600"     },
        ].map((s) => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/40">
              <tr>
                <th className="text-start font-medium text-muted-foreground px-4 py-3 text-xs">{t("table.employee")}</th>
                <th className="text-start font-medium text-muted-foreground px-4 py-3 text-xs">{t("table.trade")}</th>
                <th className="text-start font-medium text-muted-foreground px-4 py-3 text-xs hidden sm:table-cell">{t("table.phone")}</th>
                <th className="text-end font-medium text-muted-foreground px-4 py-3 text-xs">{t("table.hourlyRate")}</th>
                <th className="text-center font-medium text-muted-foreground px-4 py-3 text-xs">{t("table.cert")}</th>
                <th className="text-center font-medium text-muted-foreground px-4 py-3 text-xs">{t("table.active")}</th>
                ...
                return (
                  <tr key={emp.id} className={`hover:bg-muted/20 transition-colors ${!emp.active ? "opacity-50" : ""}`}>
                        ...
                        <div>
                          ...
                          {emp._count.timeEntries > 0 && (
                            <p className="text-[11px] text-muted-foreground mt-0.5">
                              {t("timeEntries", { count: emp._count.timeEntries })}
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    <td className="px-4 py-3 text-end tabular-nums font-medium">
                      {emp.hourlyRate > 0 ? t("hourlyRateDisplay", { rate: emp.hourlyRate }) : "—"}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex justify-center">
                        <Badge variant="outline" className={`text-[10px] px-2 py-0.5 flex items-center gap-1 ${certCfg.cls}`}>
                          ...
                          {t(`certStatus.${cert}` as Parameters<typeof t>[0])}
                        </Badge>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex justify-center">
                        <Switch checked={emp.active} onCheckedChange={(v) => handleToggleActive(emp.id, v)} />
                      </div>
                    </td>
                    <td className="px-4 py-3">
